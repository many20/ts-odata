...
describe('tsdata', () => {
    ...
    describe('Order By', function() {
        ...
        describe('resets to default', function() {
            it('without default order', function() {
                ...
                expect(j.toString()).to.equal('http://foo.bar?$orderby=OtherValue%20asc');

                j.resetOrderBy();
                expect(j.toString()).to.equal('http://foo.bar?$orderby=CustomerName%20desc');
            });
            ...
            it('with default order', function() {
                ...
                expect(j.toString()).to.equal('http://foo.bar?$orderby=OtherValue%20desc');

                j.resetOrderBy();
                expect(j.toString()).to.equal('http://foo.bar?$orderby=CustomerName%20asc');
            });
        });
    ...
    describe('Format - ', function() {
        it('Atom', function() {
            let j = new Tso('http://foo.bar').format.atom();
            expect(j.toString()).to.equal('http://foo.bar?$format=atom');
        });

        it('Xml', function() {
            let j = new Tso('http://foo.bar').format.xml();
            expect(j.toString()).to.equal('http://foo.bar?$format=xml');
        });

        it('Json', function() {
            let j = new Tso('http://foo.bar').format.json();
            expect(j.toString()).to.equal('http://foo.bar?$format=json');
        });

        it('Custom', function() {
            let j = new Tso('http://foo.bar').format.custom('text/csv');
            expect(j.toString()).to.equal('http://foo.bar?$format=text/csv');
        });

        describe('Default is', function() {
            it('Atom', function() {
                let j = new Tso('http://foo.bar').formatDefault.atom();
                expect(j.toString()).to.equal('http://foo.bar?$format=atom');
            });

            it('Xml', function() {
                let j = new Tso('http://foo.bar').formatDefault.xml();
                expect(j.toString()).to.equal('http://foo.bar?$format=xml');
            });

            it('Json', function() {
                let j = new Tso('http://foo.bar').formatDefault.json();
                expect(j.toString()).to.equal('http://foo.bar?$format=json');
            });

            it('Custom', function() {
                let j = new Tso('http://foo.bar').formatDefault.custom('text/csv');
                expect(j.toString()).to.equal('http://foo.bar?$format=text/csv');
            });
        });
    ...
    describe('Inline Count - ', function() {
        it('All Pages', function() {
            let j = new Tso('http://foo.bar').inlineCount.allPages();
            expect(j.toString()).to.equal('http://foo.bar?$inlinecount=allpages');
        });

        it('None', function() {
            let j = new Tso('http://foo.bar').inlineCount.none();
            expect(j.toString()).to.equal('http://foo.bar?$inlinecount=none');
        });

        describe('Default is', function() {
            it('All Pages', function() {
                let j = new Tso('http://foo.bar').inlineCountDefault.allPages();
                expect(j.toString()).to.equal('http://foo.bar?$inlinecount=allpages');
            });

            it('None', function() {
                let j = new Tso('http://foo.bar').inlineCountDefault.none();
                expect(j.toString()).to.equal('http://foo.bar?$inlinecount=none');
            });
        });
    ...
    describe('Filter', function() {
        describe('Building and/or Filters', function() {
            it('single filter', function() {
                ...
                expect(j.toString()).to.equal('http://foo.bar?$filter=CustomerId%20eq%201');
            });

            it('multiple and filters', function() {
                let j = new Tso('http://foo.bar');
                j.andFilter(new FilterClause('Property1').eq(5)).andFilter(new FilterClause('Property2').eq(10));

                expect(j.toString()).to.equal('http://foo.bar?$filter=Property1%20eq%205%20and%20Property2%20eq%2010');
            });

            it('multiple or filters', function() {
                let j = new Tso('http://foo.bar');
                j.orFilter(new FilterClause('Property1').eq(5)).orFilter(new FilterClause('Property2').eq(10));

                expect(j.toString()).to.equal('http://foo.bar?$filter=Property1%20eq%205%20or%20Property2%20eq%2010');
            });
            ...
            it('mixing and/or filters', function() {
                ...
                expect(j.toString()).to.equal('http://foo.bar?$filter=p1%20eq%201%20and%20p2%20eq%205%20or%20p3%20eq%2010');
            });
            ...
            describe('Default Filters', function() {
                ...
                it('multiple and filters', function() {
                    let j = new Tso('http://foo.bar');
                    j.defaultAndFilter(new FilterClause('Property1').eq(5)).defaultAndFilter(new FilterClause('Property2').eq(10));
                ...
                it('multiple or filters', function() {
                    let j = new Tso('http://foo.bar');
                    j.defaultOrFilter(new FilterClause('Property1').eq(5)).defaultOrFilter(new FilterClause('Property2').eq(10));
                ...
                it('mixing and/or filters', function() {
                    let j = new Tso('http://foo.bar');
                    j
                        .defaultFilter(new FilterClause('p1').eq(1))
                ...
                it('mixing defaults and normal filters', function() {
                    let j = new Tso('http://foo.bar');
                    j.defaultFilter(new FilterClause('Id').eq(1)).filter(new FilterClause('Name').eq('bob'));

                    expect(j.toString()).to.equal("http://foo.bar?$filter=Id%20eq%201%20and%20Name%20eq%20'bob'");
                });
        ...
        describe('Removing Single Filters', function() {
            ...
            it('removing a logical operator filter', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('CustomerName').eq('bob')).andFilter(new FilterClause('CustomerId').eq(1));

                expect(j.toString()).to.equal("http://foo.bar?$filter=CustomerName%20eq%20'bob'%20and%20CustomerId%20eq%201");
            ...
            it('removing a arithmetic method filter', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('CustomerName').eq('bob')).andFilter(new FilterClause('Price').add(5).eq(1));

                expect(j.toString()).to.equal("http://foo.bar?$filter=CustomerName%20eq%20'bob'%20and%20Price%20add%205%20eq%201");
            ...
            it('removing date function filter', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('CustomerName').eq('bob')).andFilter(new FilterClause('Birthday').day().eq(2));

                expect(j.toString()).to.equal("http://foo.bar?$filter=CustomerName%20eq%20'bob'%20and%20day(Birthday)%20eq%202");
            ...
            it('removing math function filter', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('CustomerName').eq('bob')).andFilter(new FilterClause('Price').round().eq(2));

                expect(j.toString()).to.equal("http://foo.bar?$filter=CustomerName%20eq%20'bob'%20and%20round(Price)%20eq%202");
        ...
        describe('Type Casts', function() {
            it('cast to datetime', function() {
                ...
                expect(j.toString()).to.equal("http://foo.bar?$filter=DateAdded%20eq%20datetime'13-03-01'");
            });

            it('cast to decimal', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Price').eq(Tso.decimal(392.52)));
                expect(j.toString()).to.equal('http://foo.bar?$filter=Price%20eq%20392.52m');
            });

            it('cast to guid', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('CustomerGuid').eq(Tso.guid('1225c695-cfb8-4ebb-aaaa-80da344efa6a')));
            ...
            it('cast to single', function() {
                ...
                expect(j.toString()).to.equal('http://foo.bar?$filter=Price%20eq%20392.52f');
            });

            it('cast to double', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Price').eq(Tso.double(392.52)));
                expect(j.toString()).to.equal('http://foo.bar?$filter=Price%20eq%20392.52d');
            });

            it('string that ends with m', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Currency').eq('slurm'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=Currency%20eq%20'slurm'");
            });

            it('string that ends with f', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Currency').eq('stuff'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=Currency%20eq%20'stuff'");
            });

            it('string that ends with d', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Currency').eq('usd'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=Currency%20eq%20'usd'");
            });
        });
        ...
        describe('Logical Operators', function() {
            ...
            it('Not Equals - string', function() {
                ...
                expect(j.toString()).to.equal("http://foo.bar?$filter=CustomerName%20ne%20'bob'");
            });

            it('Not Equals - number', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('CustomerId').ne(1));
                expect(j.toString()).to.equal('http://foo.bar?$filter=CustomerId%20ne%201');
            });
        ...
        describe('Precedence Groups', function() {
            it('creating a precedence group', function() {
                let j = new Tso('http://foo.bar');
                let group = new PrecedenceGroup(new FilterClause('Name').eq('Bob'));
                j.filter(group);
                expect(j.toString()).to.equal("http://foo.bar?$filter=(Name%20eq%20'Bob')");
            });

            it('and/or with precedence groups', function() {
                let j = new Tso('http://foo.bar');
                let group = new PrecedenceGroup(new FilterClause('Name').eq('Bob')).orFilter(new FilterClause('Name').eq('George'));
                j.filter(group);

                expect(j.toString()).to.equal("http://foo.bar?$filter=(Name%20eq%20'Bob'%20or%20Name%20eq%20'George')");
            });
        ...
        describe('Arithmetic Methods', function() {
            it('Add', function() {
                ...
                expect(j.toString()).to.equal('http://foo.bar?$filter=Price%20add%205%20eq%2010');
            });

            it('Sub', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Price').sub(5).eq(10));
                expect(j.toString()).to.equal('http://foo.bar?$filter=Price%20sub%205%20eq%2010');
            });

            it('Mul', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Price').mul(5).eq(10));
                expect(j.toString()).to.equal('http://foo.bar?$filter=Price%20mul%205%20eq%2010');
            });

            it('Div', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Price').div(5).eq(10));
                expect(j.toString()).to.equal('http://foo.bar?$filter=Price%20div%205%20eq%2010');
            });

            it('Mod', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Price').mod(5).eq(10));
                expect(j.toString()).to.equal('http://foo.bar?$filter=Price%20mod%205%20eq%2010');
            });
        });
        ...
        describe('String Functions', function() {
            it('Substringof', function() {
                ...
                expect(j.toString()).to.equal("http://foo.bar?$filter=substringof('test',PropertyName)%20eq%20true");
            });
            ...
            it('Substringof - with toLower', function() {
                ...
                expect(j.toString()).to.equal("http://foo.bar?$filter=substringof('test',tolower(PropertyName))%20eq%20true");
            });
            ...
            it('Substringof - with toUpper', function() {
                ...
                expect(j.toString()).to.equal("http://foo.bar?$filter=substringof('test',toupper(PropertyName))%20eq%20true");
            });
            ...
            it('Substringof - with trim', function() {
                ...
                expect(j.toString()).to.equal("http://foo.bar?$filter=substringof('test',trim(PropertyName))%20eq%20true");
            });

            it('Endswith', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').endswith('test').eq(true));
                expect(j.toString()).to.equal("http://foo.bar?$filter=endswith(PropertyName,'test')%20eq%20true");
            });

            it('Startswith', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').startswith('test').eq(true));
                expect(j.toString()).to.equal("http://foo.bar?$filter=startswith(PropertyName,'test')%20eq%20true");
            });

            it('Length', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').length().eq(10));
                expect(j.toString()).to.equal('http://foo.bar?$filter=length(PropertyName)%20eq%2010');
            });

            it('Indexof', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').indexof('test').eq(1));
                expect(j.toString()).to.equal("http://foo.bar?$filter=indexof(PropertyName,'test')%20eq%201");
            });

            it('Replace', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').replace('test', 'bob').eq('bob'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=replace(PropertyName,'test','bob')%20eq%20'bob'");
            });

            it('Substring - without length', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').substring(1).eq('test'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=substring(PropertyName,1)%20eq%20'test'");
            });
            it('Substring - with length', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').substring(1, 2).eq('test'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=substring(PropertyName,1,2)%20eq%20'test'");
            });

            it('To Lower', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').toLower().eq('test'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=tolower(PropertyName)%20eq%20'test'");
            });

            it('To Upper', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').toUpper().eq('TEST'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=toupper(PropertyName)%20eq%20'TEST'");
            });

            it('Trim', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('PropertyName').trim().eq('test'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=trim(PropertyName)%20eq%20'test'");
            });
        });
        ...
        describe('Concatenation', function() {
            it('with nesting', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('').concat(new Concat('FirstName', 'LastName')).eq('BobSmith'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=concat(FirstName,LastName)%20eq%20'BobSmith'");
            });

            it('with nesting', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('').concat(new Concat(new Concat('City', Tso.literal(', ')), 'State')).eq('Birmingham, Alabama'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=concat(concat(City,',%20'),State)%20eq%20'Birmingham,%20Alabama'");
            });
        });
        ...
        describe('Date Functions', function() {
            it('Day', function() {
                ...
                expect(j.toString()).to.equal('http://foo.bar?$filter=day(Birthday)%20eq%202');
            });

            it('Hour', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Birthday').hour().eq(2));
                expect(j.toString()).to.equal('http://foo.bar?$filter=hour(Birthday)%20eq%202');
            });

            it('Minute', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Birthday').minute().eq(2));
                expect(j.toString()).to.equal('http://foo.bar?$filter=minute(Birthday)%20eq%202');
            });

            it('Month', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Birthday').month().eq(2));
                expect(j.toString()).to.equal('http://foo.bar?$filter=month(Birthday)%20eq%202');
            });

            it('Second', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Birthday').second().eq(2));
                expect(j.toString()).to.equal('http://foo.bar?$filter=second(Birthday)%20eq%202');
            });

            it('Year', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Birthday').year().eq(2));
                expect(j.toString()).to.equal('http://foo.bar?$filter=year(Birthday)%20eq%202');
            });
        });
        ...
        describe('Math Functions', function() {
            it('Round', function() {
                ...
                expect(j.toString()).to.equal('http://foo.bar?$filter=round(Price)%20eq%202');
            });

            it('Floor', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Price').floor().eq(2));
                expect(j.toString()).to.equal('http://foo.bar?$filter=floor(Price)%20eq%202');
            });

            it('Ceiling', function() {
                let j = new Tso('http://foo.bar');
                j.filter(new FilterClause('Price').ceiling().eq(2));
                expect(j.toString()).to.equal('http://foo.bar?$filter=ceiling(Price)%20eq%202');
            });
        });
        ...
        describe('Capture Filter', function() {
            it('Apply Filter, then capture, add more filters, then reset to capture', function() {
                ...
                expect(j.toString()).to.equal("http://foo.bar?$filter=Status%20eq%20'Pending'");

                j.captureFilter();
                j.andFilter(new FilterClause('Name').eq('Chris'));
                expect(j.toString()).to.equal("http://foo.bar?$filter=Status%20eq%20'Pending'%20and%20Name%20eq%20'Chris'");
        ...
        // TODO add test for 'isOf' and 'any' and 'all'

        describe('Count - ', function() {
            it('count', function() {
                let j = new Tso('http://foo.bar').count();
        ...
        // TODO: add tests for ExpandClause

        describe('call - ', function() {
            it('set CallFunction', function() {
                let j = new Tso('http://foo.bar').setCallFunction(queryString => {
                    // call ajex request with queryString
                    let responseFromRequest = { data: 'bla' };
                    return responseFromRequest;
                });

                expect(j.call()).to.deep.equal({ data: 'bla' });
            });
